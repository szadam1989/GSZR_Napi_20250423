...
(LAST_RUNNING <- sqlQuery(channelOracle, "select * from VB_REP.VB_APP_INIT where ALKALMAZAS like 'MNB napi%' and PROGRAM = 'mnb_EBEAD.sql' and PARAM_NEV = 'utolso_futas'")) #  változáslista küldése
#2025-04-22 10:30:00
#LAST_RUNNING$PARAM_ERTEK <- "2025-04-22 10:30:00"

CHANGED_ON_20250423 <- sqlQuery(channelOracle, "select * from VB_REP.MNB_NAPI") # _DEBUG
View(CHANGED_ON_20250423[CHANGED_ON_20250423$KOD == "Q01",])
View(CHANGED_ON_20250423[CHANGED_ON_20250423$KOD == "Q02",])
View(CHANGED_ON_20250423[CHANGED_ON_20250423$KOD == "Q03",])
...
CHANGED_M0783_R <- sqlQuery(channelOracle, paste("select M003, M0783_R from VB.F003 where M0783_R >= TO_DATE('", LAST_RUNNING$PARAM_ERTEK, "', 'YYYY/MM/DD HH:MI:SS') and substr(M0491, 1, 2) != '23' and M0491 != '961' and M0491 != '811'"))
CHANGED_M0583_R <- sqlQuery(channelOracle, paste("select M003, M0583_R from VB.F003 where M0583_R >= TO_DATE('", LAST_RUNNING$PARAM_ERTEK, "', 'YYYY/MM/DD HH:MI:SS') and substr(M0491, 1, 2) != '23' and M0491 != '961' and M0491 != '811'"))
...
CHANGED_HIST_ALAKDAT <- sqlQuery(channelOracle, paste("select M003, datum, alakdat_u from VB.F003_HIST3PR where datum >= TO_DATE('", LAST_RUNNING$PARAM_ERTEK, "', 'YYYY/MM/DD HH:MI:SS') and alakdat != alakdat_u and M003 in (select M003 from VB.F003 where substr(M0491, 1, 2) != '23' and M0491 != '961' and M0491 != '811')"))
